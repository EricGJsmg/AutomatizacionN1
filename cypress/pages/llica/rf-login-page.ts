class RFLoginPage {
  ...
  radioOk(): Cypress.Chainable<any> {
    return cy
      .get('#P1_RADIO_SELEC')
      .should('exist')
      .should('be.visible') // type('', { delay: 50 })
      .get('#P1_CONFIRMAR')
      .click(); // click on OK button
  }

  submit(): Cypress.Chainable<any> {
    return cy.get('#P101_LOGIN').click(); // login button
  }

  login({ username = Cypress.env('username'), password = Cypress.env('password') } = {}): Cypress.Chainable<any> {
    return cy
      .then(() => this.visit())
